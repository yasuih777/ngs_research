...
if [ "${command}" = "gene_expression" ]; then
    ...
    curl -o ${utils_data_dir}${species_reference_genome}/reference/${species_name}.IRGSP-1.0.cdna.all.fa.gz \
        ${URL}fasta/oryza_sativa/cdna/${species_name}.IRGSP-1.0.cdna.all.fa.gz

    # unzip
    gunzip ${utils_data_dir}${species_reference_genome}/reference/${species_name}.IRGSP-1.0.cdna.all.fa.gz
else
